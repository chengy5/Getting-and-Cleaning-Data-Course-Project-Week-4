...
#Read features data
features <-read.table("UCI HAR Dataset/features.txt", header = F,strip.white=TRUE, stringsAsFactors=FALSE) 

#Read test data and merge test data into one data set
subject_test <-read.table("UCI HAR Dataset/test/subject_test.txt", 
                          header = F, col.names = "VolunteerID")
x_test <-read.table("UCI HAR Dataset/test/X_test.txt", 
                    header = F, col.names = features[1:561,2])
y_test <-read.table("UCI HAR Dataset/test/y_test.txt", 
                    header = F, col.names = "Activity", colClasses = "factor")
